...
import './Headerser.css';

const HeaderSer = () => {
    const { servicehead } = FecthHook();
    const { register, handleSubmit } = useForm();
    const onSubmit = data => {
        fetch(`http://localhost:5000/serviceheaderUpdate/${data._id}`, {
            method: 'PUT',
            headers: {
            ...
            <Container>
                <Row>      {/* header section */}
                    <div className="header-section pb-5">
                        ...
                        {
                            servicehead.map(item => <form key={item._id} onSubmit={handleSubmit(onSubmit)} className='mobiledevice p-3'>
